...
def load_parquet_data(filepath: str,
                      timestamp_col_name_in_file: str = 'timestamp', # Actual name of time col in Parquet if not index
                      standard_timestamp_col: str = 'timestamp', # Standard name for index in output df
                      column_name_map: Optional[Dict[str, str]] = None, # Maps standard_name -> name_in_file
                      columns_to_load: Optional[List[str]] = None # List of actual column names in file to load
                     ) -> Optional[pd.DataFrame]:
    """
    Loads data from a Parquet file into a pandas DataFrame, standardizing column names.

    Args:
        filepath (str): Path to the Parquet file.
        timestamp_col_name_in_file (str): The actual name of the timestamp column in the Parquet file
                                          if it's not already the index. This column will be set
                                          as the index. If the Parquet file already has a DateTimeIndex,
                                          this argument is used to verify its name or rename it.
        standard_timestamp_col (str): The desired name for the DatetimeIndex in the output DataFrame.
        column_name_map (Optional[Dict[str, str]]): A dictionary mapping internal standard column names
                                                   (keys) to the actual column names in the Parquet file (values).
                                                   Example: {'flow_rate': 'Flow.40ms', 'pressure': 'Press.2s'}
        columns_to_load (Optional[List[str]]): Specific actual column names to load from the Parquet file.
                                               If None, all columns are loaded. If provided,
                                               `timestamp_col_name_in_file` and columns mentioned in
                                               `column_name_map` values should be included if they are desired.
    Returns:
        Optional[pd.DataFrame]: DataFrame with a DatetimeIndex (named as `standard_timestamp_col`)
                                and standardized column names, or None if loading fails.
    """
    try:
    ...
    if df.empty:
        print(f"Warning: Loaded Parquet file is empty from {filepath}")
        return df

    # Handle Timestamp and Index
    # Scenario 1: Parquet file's index is already a DatetimeIndex
    if isinstance(df.index, pd.DatetimeIndex):
        print(f"Info: Parquet file has a DatetimeIndex named '{df.index.name}'.")
        if df.index.name != standard_timestamp_col:
            print(f"Renaming index to '{standard_timestamp_col}'.")
            df.index.name = standard_timestamp_col
    # Scenario 2: A column needs to be converted and set as index
    elif timestamp_col_name_in_file in df.columns:
        try:
            print(f"Converting column '{timestamp_col_name_in_file}' to DatetimeIndex named '{standard_timestamp_col}'.")
            df[standard_timestamp_col] = pd.to_datetime(df[timestamp_col_name_in_file])
            df = df.set_index(standard_timestamp_col)
            if timestamp_col_name_in_file != standard_timestamp_col: # Drop original if different name
                df = df.drop(columns=[timestamp_col_name_in_file])
        except Exception as e:
            print(f"Error converting column '{timestamp_col_name_in_file}' to DatetimeIndex: {e}")
            return None
    else:
        print(f"Error: Timestamp column '{timestamp_col_name_in_file}' not found in Parquet file columns or as index.")
        print(f"Available columns: {df.columns.tolist()}. Index type: {type(df.index)}")
        return None

    # Ensure index is sorted
    if not df.index.is_monotonic_increasing:
        df = df.sort_index()
        print("Info: DataFrame index was not sorted and has been sorted.")

    # Standardize column names using the provided map
    if column_name_map:
        # actual_to_standard_map = {v: k for k, v in column_name_map.items() if v in df.columns}
        # df = df.rename(columns=actual_to_standard_map)

        # Select only the columns that are successfully mapped and keep them in a defined order if possible
        # Standard columns expected by the rest of the application:
        # (This can be made more dynamic or passed as an argument)
        desired_standard_cols = ['flow_rate', 'pressure', 'leak_rate',
                                 'minute_ventilation', 'respiratory_rate', 'tidal_volume', 'flow_limitation']
                                 # Added flow_limitation as it seems present from Parquet

        processed_df = pd.DataFrame(index=df.index)
        for std_name, file_name in column_name_map.items():
            if file_name in df.columns:
                processed_df[std_name] = df[file_name]
            # else:
                # print(f"Warning: Column '{file_name}' (mapped to '{std_name}') not found in Parquet file.")

        # Ensure all desired standard columns are present, fill with NaN if source was missing
        for std_col in desired_standard_cols:
            if std_col not in processed_df.columns:
                 # print(f"Info: Standard column '{std_col}' not in mapped data, will be NaN if not mapped from source.")
                 # This case should ideally be handled by ensuring column_name_map is comprehensive
                 # or by accepting that some columns might be missing.
                 pass # Let it be missing if not in map; feature engineering should handle missing features.
        df = processed_df
